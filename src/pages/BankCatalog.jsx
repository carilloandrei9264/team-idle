...
const BANKS = ["BDO", "Landbank", "Metrobank"];
...
export default function BankCatalog() {
  ...
  const results = useMemo(() => properties.filter((property) => {
    ...
    if (bank && property.bank !== bank) return false;
    ...
    <div className="user-page">
      ...
      <main className="user-page__content bank-catalog-page">
        ...
        <section className="bank-catalog__filters" aria-label="Filter bank properties">
          <div className="field"><label className="field__label" htmlFor="bank-filter">Bank</label><select id="bank-filter" className="field__input" value={bank} onChange={(event) => setBank(event.target.value)}><option value="">All banks</option>{BANKS.map((name) => <option key={name}>{name}</option>)}</select></div>
