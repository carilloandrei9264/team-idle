...
import { collection, doc, onSnapshot, query, updateDoc, serverTimestamp, where, getDocs } from "firebase/firestore";
...
export default function BookingRequests() {
  ...
  async function decide(request, status) {
    ...
    try {
      if (status === "Confirmed") {
        const snapshot = await getDocs(query(collection(db, "bookings"), where("listingId", "==", request.listingId), where("status", "==", "Confirmed")));
        const conflict = snapshot.docs.some((item) => item.id !== request.id && bookingDatesOverlap(item.data(), request.startDate.toDate(), request.endDate.toDate()));
