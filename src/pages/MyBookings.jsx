...
export default function MyBookings() {
  ...
  useEffect(() => onSnapshot(
    ...
    (snapshot) => {
      setBookings(snapshot.docs.map((item) => ({ id: item.id, ...item.data() })).sort(sortByCreatedAt));
      setLoading(false);
    },
  ...
  return (
    <div className="user-page">
      ...
      <main className="user-page__content">
        ...
        {loading ? <p className="user-page__empty">Loading your bookings...</p> : bookings.length === 0 ? <div className="user-page__empty"><h2>No bookings yet</h2><p>When you request a stay, its status will appear here.</p><Link to="/browse" className="btn btn--primary">Browse listings</Link></div> : (
          <div className="user-page__list">
            {bookings.map((booking) => <article className="user-page__item booking-item" key={booking.id}>
